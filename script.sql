...
-- copying INSERT from projectdata.sql
/* 
 --------------------------------------------
...
--Querying to display category, dish_name, price sorted by dish name
...
--Querying to display category, dish_name, price sorted by category name
SELECT category.name AS category, dish.name AS dish_name, categories_dishes.price FROM category, dish, categories_dishes
WHERE categories_dishes.dish_id = dish.id AND categories_dishes.category_id = category.id
ORDER BY category.name;

--Querying to display all spicy dishes, their prices and category
...
--Querying to display the dish_id, the count dish_id form the categories_dishes table
SELECT dish_id, COUNT(dish_id) AS dish_count FROM categories_dishes
GROUP BY 1 ORDER BY 1;

--Querying to display the only the dishes form the categories_dishes table
SELECT dish_id, COUNT(dish_id) AS dish_count FROM categories_dishes
GROUP BY 1 HAVING COUNT(dish_id) > 1 ORDER BY 1;

--Querying to display multiple names and dish counts
...
--Querying to display the best ratings and its description
